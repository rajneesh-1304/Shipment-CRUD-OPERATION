...
import { PickupController } from 'src/feature/stops/pickup/pickup.controller';
...
describe('PickupController', () => {
    ...
    it('should throw error if ids are missing', async () => {
        ...
        expect(mockService.pickup).toHaveBeenCalledWith('', '', tenantName);
    });
    ...
    it('should throw error if shipment not found', async () => {
        ...
        await expect(
            controller.pickup(shipmentId, stopId, request)
    ...
    it('should complete pickup successfully', async () => {
        const stop = new StopMother({type: StopType.PICKUP,
        ...
        const result = await controller.pickup(shipmentData.id,stop.id,request);
